...
function renderCars() {
  ...
  if (searchQuery) {
    ...
    filteredCars = filteredCars.filter(car => car.make && car.make.toLowerCase().includes(query) || car.model && car.model.toLowerCase().includes(query) || car.category && car.category.toLowerCase().includes(query) || car.condition && car.condition.toLowerCase().includes(query));
  }
...
window.openDetails = function (carId) {
  ...
  const stockInfoHtml = car.status !== 'sold' && !isOutOfStock ? `<span class="car-badge" style="position:relative; top:0; left:0; background: ${qty <= 3 ? 'rgba(255,165,0,0.2); border-color: #FFA500; color: #FFA500;' : 'rgba(76,175,80,0.2); border-color: #4CAF50; color: #4CAF50;'}">${qty <= 3 ? 'Only ' + qty + ' left!' : qty + ' in stock'}</span>` : '';
